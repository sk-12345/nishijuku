...
$role = requirePermission($pdo, 'practice_flg');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === "update") {
        ...
        try {
            ...
            $stmt = $pdo->prepare("UPDATE practices SET title=?, description=?, update_datetime=NOW(), update_user_id=?, update_func_id='practice_post', lock_timestamp=CURRENT_TIMESTAMP WHERE id=?");
            $stmt->execute([$title, $description, $audit_user, $id]);
            $check = $pdo->prepare("SELECT id FROM practices WHERE id=?");
            $check->execute([$id]);
            if (!$check->fetchColumn()) throw new RuntimeException('practice_not_found');
            savePhotoManifest($pdo, 'practice_images', 'practice_id', $id, $UPLOAD_DIR_REAL, 'practice', $audit_user, 'practice_post');
            $pdo->commit();
    ...
    if ($action === "add") {
        ...
        $stmt = $pdo->prepare("
            ...
            VALUES(?,?,NOW(),?,'practice_post',NOW(),?,'practice_post',CURRENT_TIMESTAMP)
        ");
        ...
        savePhotoManifest($pdo, 'practice_images', 'practice_id', (int)$practice_id, $UPLOAD_DIR_REAL, 'practice', $audit_user, 'practice_post');
        ...
        if(isset($_FILES['images'])){
            ...
            foreach($_FILES['images']['tmp_name'] as $i=>$tmp){
                ...
                $stmt = $pdo->prepare("
                    ...
                    (practice_id, description, image_path, display_order,
                     append_datetime, append_user_id, append_func_id,
                     update_datetime, update_user_id, update_func_id, lock_timestamp)
                    VALUES(?,?,?,?,NOW(),?,'practice_post',NOW(),?,'practice_post',CURRENT_TIMESTAMP)
                ");
